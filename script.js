...
property[6] = "Pressure & Stress";
unit[6] = new Array("Newton/sq.meter", "Atmosphere (normal)", "Atmosphere (techinical)", "Bar", "Centimeter mercury(cmHg)", "Centimeter water (4'C)", "Decibar", "Kgr force/sq.centimeter", "Kgr force/sq.meter", "Kip/square inch", "Millibar", "Millimeter mercury(mmHg)", "Pascal (Pa)", "Kilopascal (kPa)", "Megapascal (Mpa)", "Poundal/sq.foot", "Pound-force/sq.foot", "Pound-force/sq.inch (psi)", "Torr (mmHg,0'C)");
factor[6] = new Array(1, 101325, 98066.5, 100000, 1333.22, 98.0638, 10000, 98066.5, 9.80665, 6894757, 100, 133.3224, 1, 1000, 1000000, 47.88026, 47.88026, 6894.757, 133.322);

/* VOLUME & CAPACITY */

property[7] = "Volume & Capacity";
unit[7] = new Array("Cubic Meter (m^3)", "Cubic centimeter", "Cubic millimeter", "Acre-foot", "Barrel (oil)", "Board foot", "Bushel (US)", "Cup", "Fluid ounce (US)", "Cubic foot", "Gallon (UK)", "Gallon (US,dry)", "Gallon (US,liq)", "Gill (UK)", "Gill (US)", "Cubic inch (in^3)", "Liter (new)", "Liter (old)", "Ounce (UK,fluid)", "Ounce (US,fluid)", "Peck (US)", "Pint (US,dry)", "Pint (US,liq)", "Quart (US,dry)", "Quart (US,liq)", "Stere", "Tablespoon", "Teaspoon", "Ton (register)", "Cubic yard");
factor[7] = new Array(1, .000001, .000000001, 1233.482, .1589873, .002359737, .03523907, .0002365882, .00002957353, .02831685, .004546087, .004404884, .003785412, .0001420652, .0001182941, .00001638706, .001, .001000028, .00002841305, .00002957353, 8.8097680E-03, .0005506105, 4.7317650E-04, .001101221, 9.46353E-04, 1, .00001478676, .000004928922, 2.831685, .7645549);
...
property[9] = "Force";
unit[9] = new Array("Newton (N)", "Dyne (dy)", "Kilogram force (kgf)", "Kilopond force (kpf)", "Kip (k)", "Ounce force (ozf)", "Pound force (lbf)", "Poundal");
factor[9] = new Array(1, .00001, 9.806650, 9.806650, 4448.222, .2780139, .4535924, .138255);

// FUnctions


  // updating the unit menu corresponding to propmenu

  function UpdateUnitMenu(propMenu, unitMenu) {
    var i;
    i = propMenu.selectedIndex;
    ArrayMenu(unitMenu, unit[i]);
  }
  
  function ArrayMenu(myMenu, myArray) {
    var i;
  ...
  function CalculateUnit(inputForm, outputForm) {
    var inputValue = inputForm.unit_input.value;  // A simple wrapper function to validate input before making the conversion
  
    inputValue = parseFloat(inputValue);          // To check if the user has given numbers or anything 
    if (!isNaN(inputValue) || inputValue == 0) { 
      inputForm.unit_input.value = inputValue;
      ConvertFromTo(inputForm, outputForm);
    }
  }
  
  function ConvertFromTo(inputForm, outputForm) {
    var propIndex;
    var inputIndex;
    var inputFactor;
    var outputIndex;
    ...
    propIndex = document.propertyform.the_menu.selectedIndex;
  
    // determine what unit are we converting from input  and the factor needed to convert that unit to the base unit.
    inputIndex = inputForm.unit_menu.selectedIndex;
    inputFactor = factor[propIndex][inputIndex];
  
    // same thing for output unit - the units we are converting TO:
    outputIndex = outputForm.unit_menu.selectedIndex;
    outputFactor = factor[propIndex][outputIndex];
  
    //  convert the input TO the base unit: (The input has been checked by the CalculateUnit function).
    result = inputForm.unit_input.value;
    // Handle Temperature increments!
    if (property[propIndex] == "Temperature") {
      result = parseFloat(result) + tempIncrement[inputIndex];
    }
    result = result * inputFactor;
  

    // use the outputFactor to convert FROM the base unit
    // to the output unit...
    result = result / outputFactor;
    // Again, handle Temperature increments!
    if (property[propIndex] == "Temperature") {
      result = parseFloat(result) - tempIncrement[outputIndex];
    }
  
    //  to update the output's input box:
    outputForm.unit_input.value = result;
  }
  

  window.onload = function(e) {
    ArrayMenu(document.propertyform.the_menu, property);
    UpdateUnitMenu(document.propertyform.the_menu, document.form1.unit_menu);
    UpdateUnitMenu(document.propertyform.the_menu, document.form2.unit_menu)
  }
